import { AppError } from "../../../../core/errors/app-error";
import { hashPassword, comparePassword, compareToken } from "../../../../infrastructure/libs/bcrypt";
import { decodeToken, signAccessToken, signRefreshToken, storeRefreshToken } from "../../../../infrastructure/libs/jwt";
import { RegisterInput, LoginInput, VerifyRegisterInput, ResendOTPIput, RefreshTokenInput } from "./auth.schema";
import { UserStatus } from "../../../../core/enums/status";
import { sendWelcomeEmail } from "../../../../infrastructure/services/email.service";
import { generateOTP } from "../../../../core/utils/generate-code";
import { OtpType } from "../../../../core/enums/otp-type";
import { UserRole } from "../../../../core/enums/role";
import { UserFacade } from "../../../../infrastructure/facades/user.facade";
import { AuthFacade } from "../../../../infrastructure/facades/auth.facade";

export class AuthService {
    static async register(data: RegisterInput) {
        const existingEmail = await UserFacade.findByEmail(data.email);
        if (existingEmail) throw new AppError("Email already exists", 409);

        const existingPhone = data.phone ? await UserFacade.findByPhone(data.phone) : null;
        if (existingPhone) throw new AppError("Phone already exists", 409);

        const { confirmPassword, ...userData } = data;
        const hashedPassword = await hashPassword(userData.password);

        const roleUser = await UserFacade.findRole(UserRole.USER);
        if (!roleUser) throw new AppError("Default user role not found", 500);

        const user = await UserFacade.createWithRole(
            {
                email: userData.email,
                phone: userData.phone,
                password: hashedPassword,
                avatar: userData.avatar,
                fullname: userData.fullname,
                status: UserStatus.INACTIVE,
                createdAt: new Date(),
                updatedAt: new Date(),
            },
            roleUser.id
        );

        const { password, ...userWithoutPassword } = user;
        const code = generateOTP(6);

        await AuthFacade.upsertOtp(user.email, code, new Date(Date.now() + 15 * 60 * 1000));
        sendWelcomeEmail(userWithoutPassword.email!, userWithoutPassword.phone!, userWithoutPassword.fullname!, code);

        return userWithoutPassword;
    }

    private static async getUserRolesAndPermissions(userId: bigint) {
        const [roles, userPermissions] = await UserFacade.getRolesAndPermissions(userId);

        const roleNames = roles.map((r) => r.role.name);
        const rolePermissions = roles.flatMap((r) =>
            r.role.rolesPermissions.map((rp) => rp.permission.key)
        );
        const specificAccess = userPermissions.filter((up) => !up.isDeny).map((up) => up.permission.key);
        const specificDeny = userPermissions.filter((up) => up.isDeny).map((up) => up.permission.key);

        return {
            role: roleNames.length > 0 ? roleNames[0] : "user",
            permissions: {
                access: [...new Set([...rolePermissions, ...specificAccess])],
                deny: specificDeny,
            },
        };
    }

    static async login(data: LoginInput, ipAddress?: string, userAgent?: string) {
        const user = await UserFacade.findByCredentials(data.username, UserStatus.ACTIVE);
        if (!user) throw new AppError("Invalid credentials", 401);

        const isMatch = await comparePassword(data.password, user.password);
        if (!isMatch) throw new AppError("Invalid credentials", 401);

        const { role, permissions } = await this.getUserRolesAndPermissions(user.id);

        const accessToken = signAccessToken({ userId: Number(user.id), role, permissions });
        const refreshToken = signRefreshToken({ userId: Number(user.id) });

        const hasLogin = await AuthFacade.findJwtToken(user.id, ipAddress, userAgent);
        if (hasLogin) await AuthFacade.deleteJwtToken(hasLogin.id);

        await storeRefreshToken(user.id, refreshToken, ipAddress, userAgent);

        const { password, ...userWithoutPassword } = user;
        return { user: userWithoutPassword, accessToken, refreshToken };
    }

    static async resendOTP(data: ResendOTPIput) {
        const user = await UserFacade.findByEmail(data.email);
        if (!user) throw new AppError("User not found", 404);

        const code = generateOTP(6);
        await AuthFacade.upsertOtp(user.email, code, new Date(Date.now() + 15 * 60 * 1000));

        if (data.type === OtpType.REGISTER) {
            sendWelcomeEmail(user.email!, user.phone!, user.fullname!, code);
        }
    }

    static async verifyRegister(data: VerifyRegisterInput, ipAddress?: string, userAgent?: string) {
        const user = await UserFacade.findByEmail(data.email);
        if (!user) throw new AppError("User not found", 404);

        const otp = await AuthFacade.findOtpByEmail(user.email);
        if (!otp) throw new AppError("OTP not found", 404);
        if (otp.otp !== data.otp) throw new AppError("Invalid OTP", 400);
        if (!otp.expiredAt || otp.expiredAt < new Date()) throw new AppError("OTP expired", 400);

        await UserFacade.activate(user.email);
        await AuthFacade.deleteOtp(user.email);

        const { role, permissions } = await this.getUserRolesAndPermissions(user.id);

        const accessToken = signAccessToken({ userId: Number(user.id), role, permissions });
        const refreshToken = signRefreshToken({ userId: Number(user.id) });

        await storeRefreshToken(user.id, refreshToken, ipAddress, userAgent);

        const { password, ...userWithoutPassword } = user;
        return { user: userWithoutPassword, accessToken, refreshToken };
    }

    static async refreshToken(data: RefreshTokenInput, ipAddress?: string, userAgent?: string) {
        const userId = decodeToken(data.accessToken)?.userId;
        if (!userId) throw new AppError("Invalid access token", 401);

        const user = await UserFacade.findById(userId);
        if (!user) throw new AppError("User not found", 404);

        const jwtToken = await AuthFacade.findJwtToken(BigInt(userId), ipAddress, userAgent);
        if (!jwtToken) throw new AppError("Invalid refresh token", 401);

        const isMatch = await compareToken(data.refreshToken, jwtToken.refreshTokenHash);
        if (!isMatch) throw new AppError("Invalid refresh token", 401);

        if (jwtToken.expiredAt && jwtToken.expiredAt < new Date()) {
            await AuthFacade.deleteJwtTokensByDevice(BigInt(userId), ipAddress, userAgent);
            throw new AppError("Refresh token expired", 401);
        }

        const { role, permissions } = await this.getUserRolesAndPermissions(user.id);
        const accessToken = signAccessToken({ userId: Number(user.id), role, permissions });

        return { accessToken };
    }
}
